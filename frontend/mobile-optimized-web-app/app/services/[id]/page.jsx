...
export default function ServiceDetailsPage({ params }) {
  // Next.js 14+ params is a promise, unwrap with React.use()
  const { id } = React.use(params);
  console.log('ServiceDetailsPage id:', id);
  ...
  console.log('Fetched service data:', data);
        // The actual service is under data.data.service
        if (data?.data?.service) {
          setService(data.data.service);
          console.log('Service object keys:', Object.keys(data.data.service));
          console.log('Service object:', data.data.service);
        } else {
          setService(null);
        }
      } catch (err) {
        ...
        {loading ? (
          ...
          <div className="w-full max-w-md mx-auto bg-white rounded-2xl shadow-xl p-7 border border-gray-100">
            ...
            <div className="mb-4">
              ...
              <div className="text-base text-black font-medium">{service.department?.name || 'N/A'}</div>
            </div>
            {service.required_documents && (
              <div className="mb-4">
                <div className="text-sm text-gray-500 mb-1">Required Documents</div>
                <ul className="list-disc pl-5 text-base text-black">
                  {Array.isArray(service.required_documents) ? service.required_documents.map((req, i) => <li key={i}>{req}</li>) : <li>{service.required_documents}</li>}
                </ul>
              </div>
            )}
            <div className="flex flex-wrap gap-2 text-xs text-gray-500 mb-4">
              {service.category && <span className="bg-blue-50 text-blue-700 px-2 py-1 rounded">{service.category}</span>}
              {service.fee_amount && <span className="bg-green-50 text-green-700 px-2 py-1 rounded">Fee: Rs. {service.fee_amount}</span>}
              {service.estimated_duration_minutes && <span className="bg-yellow-50 text-yellow-700 px-2 py-1 rounded">Duration: {service.estimated_duration_minutes} min</span>}
            </div>
